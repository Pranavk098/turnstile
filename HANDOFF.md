...
2. **D7/D8 sensitivity sweeps** — I dispatched a subagent for `packages/experiments/sweeps.py` (it exists on disk). **Check `git status` + `.superpowers/.../sweeps-report.md`** — it may be committed, or written-but-uncommitted (subagents sometimes misreport "not a git repo"; recover by committing + reviewing). Sweeps vary the barge-in-rate (D7) and silence-gap (D8) params → magnitude-as-a-function-of-input.
